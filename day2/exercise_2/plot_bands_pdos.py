...
def load_pdos(pdos_files, e_fermi=0.0):
    ...
    pdos_files : dict of str
        ...
        filenames obtained from `projwfc.x`)
    e_fermi : float
    ...
    for label, filename in pdos_files.items():
        ...
        pdos[label] = np.loadtxt(filename, skiprows=1, usecols=(0, 1))
...
def plot_bands_pdos(HSP, k, bands, pdos, total_dos, savefig=False, ylim=None, dos_xlim=None):
    """
    ...
    for i, label in enumerate(labels):
        ...
        if '4s' in label:
            label_4s = label
            pdos_proj_4s = pdos_projs[i]

        elif '4p' in label:
            label_4p = label
            pdos_proj_4p = pdos_projs[i]

        elif '4d' in label:
            label_4d = label
            pdos_proj_4d = pdos_projs[i]

        elif '5s' in label:
            label_5s = label
            pdos_proj_5s = pdos_projs[i]

        elif '5p' in label:
            label_5p = label
            pdos_proj_5p = pdos_projs[i]

    zero = np.zeros(len(pdos_proj_4d[:, 1]))

    ax_dos.fill_betweenx(y=pdos_proj_4d[:, 0],
                         x1=zero,
                         x2=zero + pdos_proj_4d[:, 1],
    ...
    ax_dos.fill_betweenx(y=pdos_proj_5s[:, 0],
                         x1=zero,
                         x2=zero + pdos_proj_5s[:, 1],
                         facecolor='salmon',
                         label=label_5s,
                         edgecolor='black')
    ...
    ax_dos.fill_betweenx(y=pdos_proj_5p[:, 0],
                         x1=zero,
                         x2=zero + pdos_proj_5p[:, 1],
                         facecolor='purple',
                         label=label_5p,
                         edgecolor='black')
    ...
    ax_dos.legend(frameon=False, fontsize=12, loc='upper right')
    ...
    # Show or save figure
    if savefig:
        plt.savefig('Ag_bands_pdos.png', format='png', dpi=300)
    else:
        plt.show()
...
if __name__ == '__main__':
    ...
    HSP = get_HSP(HSP_labels=[r'$W$',
                              r'$L$',
                              r'$\Gamma$',
                              r'$X$',
                              r'$W$',
                              r'$\Gamma$'],
                  bands_output_filename='03_ag_pp_bands.out')

    # Read the k-points and bands from the Si.bands.dat.gnu file
    k, bands = load_bands(filename='Ag.bands.dat.gnu', e_fermi=e_fermi)

    # Read PDOS
    pdos = load_pdos(pdos_files={'4s': 'pdos/Ag.pdos.dat.pdos_atm#1(Ag)_wfc#1(s)',
                                 '4p': 'pdos/Ag.pdos.dat.pdos_atm#1(Ag)_wfc#2(p)',
                                 '4d': 'pdos/Ag.pdos.dat.pdos_atm#1(Ag)_wfc#3(d)',
                                 '5s': 'pdos/Ag.pdos.dat.pdos_atm#1(Ag)_wfc#4(s)',
                                 '5p': 'pdos/Ag.pdos.dat.pdos_atm#1(Ag)_wfc#5(p)',
                                 },
                     e_fermi=e_fermi)

    # Read total DOS
    total_dos = load_dos(filename='./pdos/Ag.pdos.dat.pdos_tot',
                         e_fermi=e_fermi)
